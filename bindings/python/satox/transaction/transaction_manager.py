...
class TransactionManager:
    ...
    def create_transaction(self, from_address: str, to_address: str, value: int, 
                         data: str = "", fee: Optional[int] = None, config: Optional[TransactionConfig] = None) -> Transaction:
        ...
        tx = Transaction(
            ...
            value=value,
            data=data,
            fee=fee if fee is not None else (config or self._config).fee,
            status="pending",
    ...
    def update_transaction_config(self, config: TransactionConfig) -> None:
        if config.fee < 0 or config.nonce < 0 or config.value < 0:
